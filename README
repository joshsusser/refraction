...
Reflection can be installed in a Rails application as a plugin.
...
Put your rules in `config/initializers/refraction_rules.rb` (see example above).
...
* :port - The server port.  Usually not needed.
* :path - The path of the URL.
* :query - Added at the end of the URL after a question mark (?)

Any URL components not set remain unchanged from the original request URL.  You can use `set` before calls to `rewrite!`, `permanent!`, or `found!` to set common values.  Subsequent methods will merge their component values into values from `set`.
.
